...
class CleanCityData extends Command
{
    ...
    protected $description = 'Nettoie les données des villes en convertissant les codes numériques en noms';

    public function handle()
    {
        $this->info('Nettoyage des données des villes...');
        ...
        foreach ($departments as $code => $name) {
            $count = City::where('department', $code)
                ->orWhere('department', $code . ' ' . $code) // Cas du code dupliqué
                ->orWhere('department', 'like', $code . '%')
        ...
        foreach ($regions as $code => $name) {
            $count = City::where('region', $code)
                ->orWhere('region', 'like', $code . '%')
                ->update(['region' => $name]);
            $updated += $count;
        }
        
        $this->info("✅ {$updated} enregistrements mis à jour");
