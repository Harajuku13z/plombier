...
class CleanSausercouverture extends Command
{
    ...
    public function handle()
    {
        ...
        $correctUrl = 'https://plombier-versailles78.fr';
        $changes = 0;
        ...
        try {
            ...
            foreach ($settings as $setting) {
                ...
                if (is_string($value) && strpos($value, 'sausercouverture.fr') !== false) {
                    $newValue = str_replace('sausercouverture.fr', 'plombier-versailles78.fr', $value);
                    DB::table('settings')
                        ->where('id', $setting->id)
            ...
            foreach ($articles as $article) {
                ...
                if (strpos($article->content ?? '', 'sausercouverture.fr') !== false) {
                    $data['content'] = str_replace('sausercouverture.fr', 'plombier-versailles78.fr', $article->content);
                    $updated = true;
                }
                if (strpos($article->meta_description ?? '', 'sausercouverture.fr') !== false) {
                    $data['meta_description'] = str_replace('sausercouverture.fr', 'plombier-versailles78.fr', $article->meta_description);
                    $updated = true;
                }
            ...
            foreach ($ads as $ad) {
                ...
                if (strpos($ad->content ?? '', 'sausercouverture.fr') !== false) {
                    $data['content'] = str_replace('sausercouverture.fr', 'plombier-versailles78.fr', $ad->content);
                    $updated = true;
                }
                if (strpos($ad->meta_description ?? '', 'sausercouverture.fr') !== false) {
                    $data['meta_description'] = str_replace('sausercouverture.fr', 'plombier-versailles78.fr', $ad->meta_description);
                    $updated = true;
                }
